<tr class="row_{{ $row_id }}" data-row_id="{{ $row_id }}">
    ...
    @endif
    <td>{!! Form::text('variations[' . $row_id . '][name]', !empty($item) ? $item->name : null, ['class' => 'form-control']) !!}{!! Form::hidden('variations[' . $row_id . '][sub_sku]', !empty($item) ? $item->sub_sku : null, ['class' => 'form-control','required']) !!}</td>
